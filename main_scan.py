...
def train(args, io):
    train_loader = DataLoader(ScanObject(h5_filename='train', num_points=args.num_points), num_workers=8,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(ScanObject(h5_filename='test', num_points=args.num_points), num_workers=8,
    ...
    for epoch in range(args.epochs):
        ...
        for data, label, seg in train_loader:
            data, label, seg = data.to(device), label.to(device).squeeze()
...
def test(args, io):
    test_loader = DataLoader(ScanObject(h5_filename='test', num_points=args.num_points),
