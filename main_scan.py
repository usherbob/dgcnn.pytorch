...
from util import cal_loss, compute_chamfer_distance, mi_loss, IOStream
...
def train(args, io):
    train_loader = DataLoader(ScanObject(h5_filename=BASE_DIR + '/data/scanobjectnn/{}'.format(args.file_name), num_points=args.num_points), num_workers=8,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(ScanObject(h5_filename=BASE_DIR + '/data/scanobjectnn/{}'.format(args.file_name.replace('training', 'test')),
    ...
    for epoch in range(args.epochs):
        ...
        for data, label, seg in train_loader:
            ...
            loss = loss_cls + loss_seg + loss_mi + args.cd_weights * loss_cd
            loss.backward()
            opt.step()
    ...
    for data, label, seg in test_loader:
        ...
        if args.visu and count % 5 == 0:
            for i in range(data.shape[0]):
                np.save(BASE_DIR + '/ckpt/scan/%s/visu/node0_%04d.npy' % (args.exp_name, count*args.test_batch_size+i), data[i, :, :].detach().cpu().numpy())
                np.save(BASE_DIR + '/ckpt/scan/%s/visu/node1_%04d.npy' % (args.exp_name, count*args.test_batch_size+i), node1[i, :, :].detach().cpu().numpy())
    test_true = np.concatenate(test_true)
    test_pred = np.concatenate(test_pred)
    test_acc = metrics.accuracy_score(test_true, test_pred)
    avg_per_class_acc = metrics.balanced_accuracy_score(test_true, test_pred)
