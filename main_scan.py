...
def _init_():
    ckpt_dir = '/opt/data/private/bob/ckpt/scan'
...
def train(args, io):
    train_loader = DataLoader(ScanObject(h5_filename='/opt/data/private/bob/data/ScanObjectNN/main_split/{}'.format(args.file_name), num_points=args.num_points), num_workers=8,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
    test_loader = DataLoader(ScanObject(h5_filename='/opt/data/private/bob/data/ScanObjectNN/main_split/{}'.format(args.file_name.replace('training', 'test')),
    ...
    for epoch in range(args.epochs):
        ...
        if test_acc >= best_test_acc:
            ...
            torch.save(model.state_dict(), '/opt/data/private/bob/ckpt/scan/%s/models/model.t7' % args.exp_name)
    ...
    for data, label, seg in test_loader:
        ...
        if args.visu and count % 5 == 0:
            for i in range(data.shape[0]):
                np.save('/opt/data/private/bob/ckpt/scan/%s/visu/node0_%04d.npy' % (args.exp_name, count*args.test_batch_size+i), data[i, :, :].detach().cpu().numpy())
                np.save('/opt/data/private/bob/ckpt/scan/%s/visu/node1_%04d.npy' % (args.exp_name, count*args.test_batch_size+i), node1[i, :, :].detach().cpu().numpy())
    test_true = np.concatenate(test_true)
    test_pred = np.concatenate(test_pred)
    test_acc = metrics.accuracy_score(test_true, test_pred)
    avg_per_class_acc = metrics.balanced_accuracy_score(test_true, test_pred)
...
if __name__ == "__main__":
    ...
    io = IOStream('/opt/data/private/bob/ckpt/scan/' + args.exp_name + '/run.log')
    io.cprint(str(args))
