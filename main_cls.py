    ...
    for data, label in test_loader:
        ...
        if args.visu:
            for i in range(data.shape[0]):
                np.save(BASE_DIR + '/ckpt/cls/%s/visu/node0_%02d_%04d.npy' % (args.exp_name, lable[i], count*args.test_batch_size+i), data[i, :, :].detach().cpu().numpy())
                np.save(BASE_DIR + '/ckpt/cls/%s/visu/node1_%02d_%04d.npy' % (args.exp_name, label[i], count*args.test_batch_size+i), node1_static[i, :, :].detach().cpu().numpy())
    test_true = np.concatenate(test_true)
    test_pred = np.concatenate(test_pred)
    test_acc = metrics.accuracy_score(test_true, test_pred)
    avg_per_class_acc = metrics.balanced_accuracy_score(test_true, test_pred)
