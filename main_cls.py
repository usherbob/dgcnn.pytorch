...
def train(args, io):
    train_loader = DataLoader(ModelNet40(partition='train', num_points=args.num_points, num_classes=args.num_classes, BASE_DIR=BASE_DIR),
    ...
    for epoch in range(args.epochs):
        ...
        for data, label in train_loader:
            ...
            for r in ret:
                loss_mi += mi_loss(r)
            loss_cd = compute_chamfer_distance(node[0], data) + compute_chamfer_distance(node[1], data) + \
                      compute_chamfer_distance(node[2], data)
