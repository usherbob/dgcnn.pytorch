    ...
    for data, label in test_loader:
        ...
        if args.visu and count % 5 == 0:
            for i in range(data.shape[0]):
                np.save('/ceph/ckpt/cls/%s/visu/node0_%04d.npy' % (args.exp_name, count*args.test_batch_size+i), data[i, :, :].detach().cpu().numpy())
                np.save('/ceph/ckpt/cls/%s/visu/node1_%04d.npy' % (args.exp_name, count*args.test_batch_size+i), node1[i, :, :].detach().cpu().numpy())
                np.save('/ceph/ckpt/cls/%s/visu/node2_%04d.npy' % (args.exp_name, count*args.test_batch_size+i), node2[i, :, :].detach().cpu().numpy())
    test_true = np.concatenate(test_true)
    test_pred = np.concatenate(test_pred)
    test_acc = metrics.accuracy_score(test_true, test_pred)
    avg_per_class_acc = metrics.balanced_accuracy_score(test_true, test_pred)
