...
class DGCNN_semseg(nn.Module):
    def __init__(self, args):
        ...
        self.conv6_m = nn.Sequential(nn.Conv1d(64, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn6,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv7_m = nn.Sequential(nn.Conv1d(args.emb_dims+64, 512, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv8_m = nn.Sequential(nn.Conv1d(512+64, 256, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.dp1 = nn.Dropout(p=args.dropout)
        self.conv9_m = nn.Conv1d(256+64, 13, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        node1, node1_feature = self.pool1(node, x1)       # num_points -> num_points//4

        x = get_graph_feature(node1_feature, k=self.k)    # (batch_size, 64, num_points//4) -> (batch_size, 64*2, num_points//4, k)
        x = self.conv3(x)                                 # (batch_size, 64*2, num_points//4, k) -> (batch_size, 64, num_points//4, k)
        x = self.conv4(x)                                 # (batch_size, 64, num_points//4, k) -> (batch_size, 64, num_points//4, k)
        x2 = x.max(dim=-1, keepdim=False)[0]              # (batch_size, 64, num_points//4, k) -> (batch_size, 64, num_points//4)

        node2, node2_feature = self.pool2(node1, x2)      # num_points // 4 -> num_points//16
        
        x = get_graph_feature(node2_feature, k=self.k)    # (batch_size, 64, num_points//16) -> (batch_size, 64*2, num_points//16, k)
        x = self.conv5(x)                                 # (batch_size, 64*2, num_points//16, k) -> (batch_size, 64, num_points//16, k)
        x3 = x.max(dim=-1, keepdim=False)[0]              # (batch_size, 64, num_points//16, k) -> (batch_size, 64, num_points//16)

        node3, node3_feature = self.pool3(node2, x3)      # num_points // 16 -> num_points//64
        #x = torch.cat((x1, x2, x3), dim=1)               # (batch_size, 64*3, num_points//64)

        x = self.conv6_m(node3_feature)                   # (batch_size, 64, num_points//64) -> (batch_size, emb_dims, num_points//64)
        # x = x.max(dim=-1, keepdim=True)[0]              # (batch_size, emb_dims, num_points) -> (batch_size, emb_dims, 1)

        x = x.repeat(1, 1, 4)                             # (batch_size, 1024, num_points//16)
        x = torch.cat((x, x3), dim=1)                     # (batch_size, 1024+64, num_points//16)
        x = self.conv7_m(x)                               # (batch_size, 1024+64, num_points//16) -> (batch_size, 512, num_points//16)
        
        x = x.repeat(1, 1, 4)                             # (batch_size, 512, num_points//4)
        x = torch.cat((x, x2), dim=1)                     # (batch_size, 512+64, num_points//4)
        x = self.conv8_m(x)                               # (batch_size, 512, num_points//4) -> (batch_size, 256, num_points//4)
        x = self.dp1(x)

        x = x.repeat(1, 1, 4)
        x = torch.cat((x, x1), dim=1)
        x = self.conv9_m(x)                               # (batch_size, 256+64, num_points) -> (batch_size, 13, num_points)
