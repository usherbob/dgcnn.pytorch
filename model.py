...
class PointNet(nn.Module):
    def __init__(self, args, output_channels=40):
        ...
        self.pool1 = IndexSelect(self.args.num_points//4, 64)
...
class PointNet_scan(nn.Module):
    def __init__(self, args, output_channels=15, seg_num_all=2):
        ...
        self.pool1 = IndexSelect(self.args.num_points//4, 128)
        self.linear1 = nn.Linear(args.emb_dims * 2, 512, bias=False)
        self.bn6 = nn.BatchNorm1d(512)
    ...
    def forward(self, x):
        ...
        return logits_cls, logits_seg, node1, node1_static, ret
...
class DGCNN_partseg(nn.Module):
    ...
    def forward(self, x, l):
        ...
        x = torch.cat((node_feature_1, node_features_agg),
...
class DGCNN_semseg(nn.Module):
    ...
    def forward(self, x):
        ...
        node_feature_1, values, idx, ret, node1_static, node1 = self.pool1(xyz, x1)
        node_features_agg = aggregate(xyz, node1_static, x1, self.k // 2)
        node_feature_1 = torch.cat((node_feature_1, node_features_agg), dim=1)
        ...
        node_feature_2, values, idx, ret, node2_static, node2 = self.pool2(node1_static, x2)
