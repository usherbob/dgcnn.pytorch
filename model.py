...
class DGCNN_partseg(nn.Module):
    def __init__(self, args, seg_num_all):
        ...
        self.conv2 = nn.Sequential(nn.Conv2d(64*2, 64, kernel_size=1, bias=False),
                                   self.bn2,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv1_p = nn.Sequential(nn.Conv1d(64*2, 128, kernel_size=1, bias=False),
                                   self.bn1_p,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv3 = nn.Sequential(nn.Conv2d(128*2, 128, kernel_size=1, bias=False),
                                   self.bn3,
        ...
        self.conv4 = nn.Sequential(nn.Conv2d(128*2, 128, kernel_size=1, bias=False),
                                   self.bn4,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv2_p = nn.Sequential(nn.Conv1d(128*2, 256, kernel_size=1, bias=False),
                                     self.bn2_p,
                                     nn.LeakyReLU(negative_slope=0.2))
        self.conv5 = nn.Sequential(nn.Conv2d(256*2, 256, kernel_size=1, bias=False),
                                   self.bn5,
        ...
        self.conv7 = nn.Sequential(nn.Conv1d(16, 64, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv8 = nn.Sequential(nn.Conv1d(args.emb_dims+64, 512, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv9 = nn.Sequential(nn.Conv1d(512+512, 256, kernel_size=1, bias=False),
                                   self.bn9,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv10 = nn.Sequential(nn.Conv1d(256+256, 128, kernel_size=1, bias=False),
                                   self.bn10,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv11 = nn.Sequential(nn.Conv1d(128+128, 64, kernel_size=1, bias=False),
                                    self.bn11,
                                    nn.LeakyReLU(negative_slope=0.2))
        self.conv12 = nn.Sequential(nn.Conv1d(64+64, 64, kernel_size=1, bias=False),
                                    self.bn12,
                                    nn.LeakyReLU(negative_slope=0.2))
        self.dp = nn.Dropout(p=args.dropout)
        self.conv13 = nn.Conv1d(64, self.seg_num_all, kernel_size=1, bias=False)
    ...
    def forward(self, x, l):
        ...
        node1 = xyz[:, :, :self.args.num_points // 4]
        node_features_agg = aggregate(xyz, node1, x1, self.k//4)
        x = torch.cat((node_feature_1, node_features_agg), dim=1) # (batch_size, 64*2, num_points//4)
        x_p1 = self.conv1_p(x)                              # (batch_size, 128, num_points//4)
        ...
        if self.args.res:
            x2 = F.leaky_relu(x2+x_p1, negative_slope=0.2) # (batch_size, 128, num_points//4)
        else:
            x2 = F.leaky_relu(x2, negative_slope=0.2)      # (batch_size, 128, num_points//4)
         
        # Pool2
        node_feature_2 = x2[:, :, :self.args.num_points//16]
        node2 = node1[:, :, :self.args.num_points//16]
        node_features_agg = aggregate(node1, node2, x2, self.k//4)
        x = torch.cat((node_feature_2, node_features_agg), dim=1)      # (batch_size, 128*2, num_points//16)
        x_p2 = self.conv2_p(x)                             # (batch_size, 256, num_points//16)
        ...
        x = self.conv8(x)                                   # (batch_size, 1088, 1) -> (batch_size, 512, 1)

        # Upsample1
        x = x.repeat(1, 1, x4.shape[-1])                    # (batch_size, 512, num_points//64)
        x = torch.cat((x, x4), dim=1)                       # (batch_size, 512+512, num_points//64)
        x = self.conv9(x)                                   # (batch_size, 512+512, num_points//64) -> (batch_size, 256, num_points//64)

        # Upsample2
        x = unpool(node3, node2, x)                         # (batch_size, 256, num_points//16)
        x = torch.cat((x, x3), dim=1)                       # (batch_size, 256+256, num_points//16)
        x = self.conv10(x)                                  # (batch_size, 256+256, num_points//16) -> (batch_size, 128, num_points//16)

        # Upsample3
        x = unpool(node2, node1, x)                         # (batch_size, 128, num_points//4)
        x = torch.cat((x, x2), dim=1)                       # (batch_size, 128+128, num_points//4)
        x = self.conv11(x)                                  # (batch_size, 128+128, num_points//4) -> (batch_size, 64, num_points//4)

        # Upsample4
        x = unpool(node1, xyz, x)                           # (batch_size, 64, num_points)
        x = torch.cat((x, x1), dim=1)                       # (batch_size, 64+64, num_points)
        x = self.conv12(x)                                  # (batch_size, 64+64, num_points) -> (batch_size, 64, num_points)
        x = self.dp(x)

        x = self.conv13(x)                                  # (batch_size, 64, num_points) -> (batch_size, seg_num_all, num_points)
...
class DGCNN_semseg(nn.Module):
    ...
    def forward(self, x):
        ...
        x = get_graph_feature(node_feature_2,
                              k=self.k // 4)  # (batch_size, 64, num_points//16) -> (batch_size, 64*2, num_points//16, k//4)
        x = self.conv5(x)  # (batch_size, 64*2, num_points//16, k//4) -> (batch_size, 64, num_points//16, k//4)
        x3 = x.max(dim=-1, keepdim=False)[
            0]  # (batch_size, 64, num_points//16, k//4) -> (batch_size, 64, num_points//16)

        x4 = self.conv6_m(x3)  # (batch_size, 64, num_points//16) -> (batch_size, emb_dims, num_points//16)

        x = self.conv8_m(x4)  # (batch_size, 1024, num_points//16) -> (batch_size, 256, num_points//16)
        x = self.dp1(x)
